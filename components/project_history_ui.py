...
def render_project_history():
    ...
    with tab2:
        ...
        def render_project_comparison_commentary(df_A, name_A, df_B, name_B, period_label, all_proj_opt):
            def format_spaced_brackets(s):
                return str(s).replace("[", "[ ").replace("]", " ]").replace("(", "( ").replace(")", " )").replace("  ", " ").strip()

            display_A = format_spaced_brackets(name_A) if name_A != all_proj_opt else t('Tất cả dự án ( A )', 'すべてのプロジェクト ( A )')
            display_B = format_spaced_brackets(name_B) if name_B != all_proj_opt else t('Tất cả dự án ( B )', 'すべてのプロジェクト ( B )')
