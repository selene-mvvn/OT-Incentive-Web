    ...
    with st.container():
        ...
        <script>
            setTimeout(() => {
                ...
                if (parentDoc.body.dataset.timelineObserverActivated === 'true') {
                    // Already activated, just trigger a scan in case new elements were added before observer caught them
                    if (parentDoc.window && parentDoc.window.applyTimelineStyles) {
                        parentDoc.window.applyTimelineStyles();
                    }
                    return;
                }
                parentDoc.body.dataset.timelineObserverActivated = 'true';
                ...
                function applyTimelineStyles() {
                    ...
                    markers.forEach((marker, index) => {
                        // Robust DOM traversal to find the card container
                        ...
                        while(current && current.tagName !== 'BODY') {
                            ...
                            if (testid === 'stVerticalBlock') {
                                let style = parentDoc.defaultView.getComputedStyle(current);
                ...
                if (parentDoc.window) parentDoc.window.applyTimelineStyles = applyTimelineStyles;
                applyTimelineStyles();
