    ...
    with col_main:
        ...
        if uploaded_file is not None:
            try:
                ...
                col_lbl, col_rad = st.columns([0.75, 5])
                with col_lbl:
                    ...
                    if len(deleted) > 0:
                        ...
                        for idx in deleted:
                            if idx < len(df1):
                                ...
                                dh = row.get('order_name', row.get('order_id', ''))
                                ot = row.get('ot_hours', '')
                                details.append(f"- :material/cancel: **{t('Đã xóa', '削除')}**: <span style='color: #9ca3af; text-decoration: line-through; text-decoration-color: black;'>{ngay} | {ten} | {dh} | Giờ OT: {ot}</span>")
