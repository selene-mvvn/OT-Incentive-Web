        ...
        if df_filtered.empty:
            ...
            for i, row in top_5.iterrows():
                ...
                html_content += (
                    ...
                    f"align-items: center; background: {bg_color}; padding: 8px 10px; border-radius: 8px; "
                    f"margin-bottom: 8px; font-size: 13px; box-shadow: 0 1px 2px rgba(0,0,0,0.02); "
                    f"border-left: 3px solid {text_color};'>"
                    f"<div style='display: flex; align-items: center; gap: 8px;'>"
                    f"<span style='font-size: 16px;'>{medal}</span>"
                    f"<span style='font-weight: 600; color: #34495e;' title='{emp_name}'>{emp_name}</span>"
                    f"</div>"
                    f"<div style='display: flex; align-items: center;'>"
                    f"<span style='font-weight: 700; color: {text_color};'>{formatted_val} {val_suffix}</span>"
                    f"{trend_html}"
                    f"</div>"
                    f"</div>"
                )
