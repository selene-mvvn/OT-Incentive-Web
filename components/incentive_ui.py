    ...
    with col_main:
        ...
        if 'last_incentive_calc' in st.session_state:
            ...
            if result.get('final_incentive', 0) > 0:
                ...
                new_total = acc + result['final_incentive']
                current_inc = result['final_incentive']
                max_goal = max(100000.0, new_total * 1.2)
                pct_start = min(100.0, (acc / max_goal) * 100)
                pct_end = min(100.0, (new_total / max_goal) * 100)
